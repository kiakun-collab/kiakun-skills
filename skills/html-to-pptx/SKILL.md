...
## 实战经验(交付前必做,源自真实工作流复盘)

- **字体必须用本机真实注册名**:`build_layout_spec.py` 枚举 Windows 注册表拿到 PowerPoint 实际识别的字体名(如 `腾讯体 W7`),并对拼写/空格差异自动校正(写成 `腾讯体W7` 会被纠正为 `腾讯体 W7`)。凭"看起来像名字"直接写会导致 PowerPoint 静默 fallback。`fontMap[].confidence<1` 或 `warnings` 有字体条目时,说明有 fallback 风险,须核对。
- **不叠伪粗体**:目标字体名已带字重 face(W7/Bold/Black…)时不再叠合成粗体。
- **内容忠实,装饰从简**:不新增 HTML 以外内容,保持页数与源一致;遵守用户指定的删减/简化(如去掉角标装饰文字、`15` 大字背景改纯色)。这类删减在会话里明确,不由脚本臆测。
- **出界检测**:`warnings` 会列出超出 1280×720 的元素(底部页脚、多列最后一行、卡片长文本、标签右边界最易越界)。
- **用户反馈截图最有操作性**:优先按截图定位到具体页码/元素/差异,再回到 `layout-spec` 改参数重建,而不是泛泛"不太像"。

### 交付前检查清单

- 页数与源 HTML 一致;未新增未要求的内容;要求删除的元素已删除。
- 字体为本机真实注册名,无 fallback(查 `fontMap`/`warnings`);无残留错误字体名与多余伪粗。
- 无文字溢出/压缩/遮挡/异常换行;标签、按钮、卡片内字号统一(autofit 已关);底部元素未越界。
- 全部页面能正常 COM 渲染;关键页(尤其用户反馈过的)肉眼复核。
