...
name: kiakun-skills
description: |
  Kiakun 的 AI Agent Skills 集合。包含小红书自动化、B站视频总结、文件夹向量化知识库、Claude Code 第三方 API 配置、GPT Image 2 VIP API 生图、图片型 PPT 可编辑复刻、游戏 UI 资产流水线。
  当用户要求操作小红书、总结 B站视频、整理文件夹为知识库、通过 CC Switch 配置 Claude Code API、通过 gpt-image-2-vip API 生成或编辑 2K/4K 图片、将图片/截图型 PPT 复刻为可编辑 PPTX，或生成、切片、验证、导入 Godot 游戏 UI 资产时触发。
---
...
8. **GPT Image 2 VIP API**（"gpt-image-2-vip / API 生图 / 2K 4K 图片 / 参考图编辑 / aifast 图片网关"）
   → 执行 `gpt-image-2-vip-api` 技能。
...
├── gpt-image-2-vip-api/      → GPT Image 2 VIP 直接 API 生图与编辑
...
### gpt-image-2-vip-api
- 触发：用户要求通过 `gpt-image-2-vip` 或 aifast.site 兼容接口生成、编辑 2K/4K 图片
- 能力：文本生图 → 多参考图编辑 → URL 参考图编辑 → 多图落盘 → 超时与重试
- 入口文件：`skills/gpt-image-2-vip-api/SKILL.md`
